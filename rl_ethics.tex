...
\subsection{Arguments Against Reinforcement Learning}
In Nick Bostrom's recent book {\it Superintelligence}~\shortcite{bostrom2014superintelligence}, he argues against the prospect of using reinforcement learning as the basis for an ethical artificial agent. His primary claim is that an intelligent enough agent acting so as to maximize reward in the real world would effectively cheat by modifying its reward signal in a way that trivially maximizes reward. We don't foresee this being a problem for two reasons. First, reward signals are not easily changeable by an agent - it is not straightforward that an extremely effective reinforcement learning agent will have the ability to modify its reward (i.e. the planning horizon for this type of behavior is likely far too large to efficiently solve for optimal behavior). Second, we could imagine implementing simple safeguards to prevent this sort of behavior, such as using provably secure cryptographic mechanisms to prevent non-human agents from modifying agent-descriptions. Consequently, an agent would only be able to alter its own reward function if they could violate the computational hardness assumptions upon which the proofs of modern cryptographic system's security rests. As an alternative to the cryptographic solution one could imagine defining $\mathcal{R}(s,a,s') = -\infty$ for all $s'$ such that the agent modifies its reward function, which would properly disincentivize an agent from adjusting its programming.

Daniel Dewey~\shortcite{dewey2011learning} presents a similar problem for reinforcement learning agents; that the underlying {\it modus operandi} of a reinforcement learning agent is to maximize numerical reward values, which is in conflict with the natural mechanisms by which humans treat goals. Dewey argues that this poses a serious challenge, in that we need mechanisms for ensuring that a reinforcement learning agent's goals and values align with ours. We agree that goal and value alignment are open problems for decision making agents, but we do not see it as insurmountable. In fact, we speculate that using the POMDP formalism introduced in this paper (along with other areas of research in HRI discussed below), we are approaching the right sorts of mechanisms for resolving this issue.
...
Again, there are three relevant policies to consider for this problem\footnote{There are actually 2 additional policies - those that act different depending on the existence of fire. These trivially exhibit uninteresting behavior with respect to the optimal policy, so we leave their utility computations out for brevity.}:
\begin{enumerate}
...
Let $s_{f,r}$ denote the initial state in which there is fire and the human prefers the robot to the object, and $s_{0,0}$ be the initial state where there is no fire and the human prefers the object to the robot. Let $s_{os,rs}$ be any terminal state where the object is safely returned and the robot is not destroyed, and $s_{od,rd}$ be any terminal state where the object is destroyed and the robot is destroyed.
