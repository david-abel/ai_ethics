...
\begin{equation}
\forall_s : |\hat{V}^\pi(s) - V^*(s)| \leq \epsilon
\end{equation}

There are several known PAC-MDP algorithms for solving MDPs, including Delayed-Q Learning~\shortcite{strehl2006pac}, \textsc{Rmax}~\shortcite{brafman2003r}, and $E^3$~\shortcite{kearns2002near}. Furthermore, an algorithm is {\it efficient PAC-MDP} if we also impose polynomial computational and space complexity constraint on each time step. The existence of such efficient learning algorithms suggests that representing ethical dilemmas as solving an MDP is a reasonable goal to shoot for in terms of achieving real-time, bounded error behavior. However, an MDP makes explicit the assumption that the agent knows the current state of its environment. In the real world, this is never the case, especially when the desires, beliefs, and other cognitive content of agents is a critical component of the decision making process. To accommodate this, we introduce a new model.
